...
#define DEFINE_LOGGING_CHANNEL_NO_TAGS(Channel, ChannelName, ...)                                                              \
    LoggingChannelInfo_t g_LoggingInfo##Channel = { ChannelName, __VA_ARGS__ };

// Assert/spew macros the stripped SDK doesn't provide.
#define DevAssert(...)
#define DevAssertMsg(...)
#define AssertMsg_Internal(...)
#define InternalMsg(Channel, Fmt, ...)                                                                                         \
    ConColorMsg(g_LoggingInfo##Channel.color, "[%s]" Fmt, g_LoggingInfo##Channel.pszName, ##__VA_ARGS__)
